...
def test_branch_I_evaluate(test_loader, model, epoch):
    ...
    reference = patch_hash_codes / torch.norm(patch_hash_codes, dim=-1, keepdim=True)  # 方差归一化，即除以各自的模
    ...
    with torch.no_grad():
        ...
        query = hashcodes / torch.norm(hashcodes, dim=-1, keepdim=True)  # 方差归一化，即除以各自的模
